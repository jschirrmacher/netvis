...
`nodes` and `texts` on the top level:
...
Nodes is a list of nodes to display, and texts a list of textual representations of connection types.

Each node therefore has an ID, which can be referenced in other nodes:

```json
{
    "nodes": [
        {"id": 1, "name": "First", "shape": "circle", "open": true},
        {"id": 2, "name": "Second", "shape": "rect", "open": true, "links": [{"type": "topics", "nodes": [1]}]}
    ]
}
```

Links are grouped by a `type`, so that it is possible to show all connections of a specific type.
...
- `details` - an URL which returns detail data for the node in JSON format. If
  no details are provided, the node data is used. It only allows for having
  the network view use only some of the node data and then dynamically load
  more data when showing the details.
...
- showNodes - Makes all nodes visible which are connected to the current node
  via links of the given type.
